...
// ── Taste twin card (overall, cross-stack) ───────────────────────────────────

function TasteTwinCard({
  twin,
}: {
  twin: NonNullable<PayoffData["tasteTwin"]>;
}) {
  const name = twin.displayName || twin.username;
  return (
    <Link
      href={`/u/${twin.username}`}
      className="rounded-[12px] border border-rk-stroke bg-rk-surface p-4 flex items-center gap-3 hover:border-rk-muted transition-colors group"
    >
      ...
      <p className="text-[12px] text-rk-muted leading-snug min-w-0">
        You rank most like{" "}
        <span className="text-rk-secondary font-[500] group-hover:text-rk-accent transition-colors">
          @{name}
        </span>{" "}
        — {twin.pct}% across {twin.sharedLists} list
        {twin.sharedLists === 1 ? "" : "s"}
      </p>
      <ChevronRight
        size={14}
        className="text-rk-muted flex-shrink-0 ml-auto group-hover:text-rk-accent transition-colors"
      />
    </Link>
  );
}
...
export default function PayoffPage(props: PayoffPageProps) {
  ...
  // Resolved after mount so server and first client render agree (no hydration
  // mismatch); true only on touch devices that support the native share sheet.
  ...
  // Fall back to the prop token: the anon /submitted page passes the route
  // share token directly, and the anon payoff payload doesn't echo it.
  const shareToken = data.shareToken ?? props.shareToken ?? null;
  const canShare = !isAnon && (!!shareToken || !!props.listId);
  // Verdict links work for anon rankers too — identity is resolved server-side
  // from the session cookie when the link is minted.
  const canShareLink = !!shareToken;
  const rankHref = `${backHref}/s`;
  const shareLabel =
    isFirst && isComplete ? S.payoff.ctaShareFirst : S.payoff.ctaShare;

  // Mint a signed verdict link for this result and share/copy it. The unfurl of
  // the returned /r/<token>/v/<ref> URL shows the personal verdict card.
  const shareVerdictLink = async (
    template: "verdict" | "hot-takes" | "crowd"
  ) => {
    if (!shareToken || linkBusy) return;
    setLinkBusy(true);
    try {
      const res = await fetch(`/api/r/${shareToken}/share-ref`, {
        method: "POST",
      ...
      // Native share sheet only on touch devices; on desktop this just copies.
      if (nativeShare) {
        try {
          await navigator.share({ url: absolute });
        } catch (e) {
          // User dismissing the native sheet is not an error.
          ...
          <>
            ...
            {data && revealed && (
              <>
                ...
                {data.tasteTwin && (
                  <div className="rk-reveal-up" style={{ animationDelay: "120ms" }}>
                    <TasteTwinCard twin={data.tasteTwin} />
                  </div>
                )}
                ...
                >
                  <>
                    <div className="flex flex-col gap-3">
                      ...
                      {canShareLink && (
                        <button
                          onClick={() => shareVerdictLink("verdict")}
                          disabled={linkBusy}
                          className="w-full py-2.5 rounded-[10px] text-[14px] font-[500] border border-rk-stroke text-rk-secondary hover:border-rk-secondary hover:text-rk-primary transition-colors cursor-pointer disabled:opacity-50"
                        >
                          {linkBusy
                            ? "Creating link…"
                            : nativeShare

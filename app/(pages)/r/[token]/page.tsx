...
import { ImageIcon, Users, Flame } from "lucide-react";
...
import { divisiveness } from "@/lib/itemDistribution";
import { MIN_ITEM_RANKERS_FOR_BADGE } from "@/lib/insightsConfig";
...
export default function SharedListPage() {
  ...
  const [goneReason, setGoneReason] = useState<"deleted" | "taken_down" | null>(
    null
  );
  ...
  // Community (aggregate) view scopes the inline distribution expander,
  // contention dots and divisive sort.
  const isCommunityView = viewMode === "community";

  // The viewer's own ranking value for an item (for the "you" marker in the
  // inline distribution). Sourced from myRanking — find the tier whose items
  // include the id and return its value.
  const yourValueForItem = (itemId: number): number | null => {
    const tier = myRanking?.tiers?.find((t) => t.items.includes(itemId));
          ...
          {activeTiers.map((tier) => {
            ...
            if (isCommunityView && sortByDivisive) {
              tierItems = [...tierItems].sort(
                (a, b) => b.distribution.sd - a.distribution.sd
              );
            }
              ...
              <React.Fragment key={tier.id}>
                ...
                >
                  ...
                  >
                    {tierItems.map((item) => {
                      const lvl =
                        item.distribution.total >= MIN_ITEM_RANKERS_FOR_BADGE
                          ? divisiveness(item.distribution.sd)
                          : "low";
                      const contention =
                        isCommunityView && lvl !== "low" ? lvl : undefined;
                      const card = (
                        <ItemCard item={item} contention={contention} />
                      );
                      return isCommunityView ? (
                        <div
                          key={item.id}
                          role="button"
                          tabIndex={0}
                ...
                {expandedItem && (
                  <div className="rounded-[10px] border border-rk-stroke bg-rk-surface px-4 py-2">
                    ...
                    <ItemDistribution
                      dist={expandedItem.distribution}

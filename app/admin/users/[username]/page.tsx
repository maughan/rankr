...
import { ShieldAlert, Eye, Loader2, ShieldBan, Clock, AlertTriangle } from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.7)" }}>
      <div className="w-full max-w-md rounded-[16px] border border-amber-800/50 bg-rk-surface p-6 flex flex-col gap-5 shadow-2xl">
        ...
        <div className="flex items-start gap-3">
          <div className="w-10 h-10 rounded-[10px] flex items-center justify-center flex-shrink-0" style={{ backgroundColor: "rgba(217,119,6,0.15)" }}>
            <Eye size={18} className="text-amber-400" />
          </div>
          <div>
            <p className="text-[15px] font-[600] text-rk-primary">Impersonate @{username}?</p>
            <p className="text-[12px] text-rk-muted mt-0.5">View-only · Audited · 30-min expiry</p>
          </div>
        </div>

        {/* Warning block */}
        <div className="rounded-[10px] border border-amber-800/40 bg-amber-900/10 px-4 py-3 flex flex-col gap-1.5">
          <p className="text-[12px] text-amber-300 font-[500]">Before you start</p>
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-[12px] text-rk-muted">Reason <span className="text-rk-tertiary">(optional but encouraged)</span></label>
          <textarea
            value={reason}
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <h1 className="text-[18px] font-[600] text-rk-primary">User detail</h1>
          </div>
          <Link
            href="/admin/users"
        ...
        {loading ? (
          ...
          <p className="text-[13px] text-rk-muted text-center py-12">User not found.</p>
        ) : (
          <>
            {/* Profile card */}
            <div className="rounded-[12px] border border-rk-stroke bg-rk-surface p-5 flex flex-col gap-4">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[15px] font-[600] text-rk-primary">@{user.username}</p>
                    {user.display_name && (
                      <p className="text-[12px] text-rk-muted">{user.display_name}</p>
                    )}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-[12px]">
                <div className="flex flex-col gap-0.5">
                  <span className="text-rk-tertiary uppercase tracking-wide text-[10px] font-[600]">Email</span>
                  <span className="text-rk-secondary">{user.email ?? "—"}</span>
                </div>
                <div className="flex flex-col gap-0.5">
                  <span className="text-rk-tertiary uppercase tracking-wide text-[10px] font-[600]">Joined</span>
                  <span className="text-rk-secondary">
                    {formatDistanceStrict(new Date(user.createdAt), new Date(), { addSuffix: true })}
                  </span>
                </div>
                <div className="flex flex-col gap-0.5">
                  <span className="text-rk-tertiary uppercase tracking-wide text-[10px] font-[600]">Lists</span>
                  <span className="text-rk-secondary">{user._count.lists}</span>
                </div>
                <div className="flex flex-col gap-0.5">
                  <span className="text-rk-tertiary uppercase tracking-wide text-[10px] font-[600]">Rankings</span>
                  <span className="text-rk-secondary">{user._count.rankings}</span>
                </div>
                <div className="flex flex-col gap-0.5">
                  <span className="text-rk-tertiary uppercase tracking-wide text-[10px] font-[600]">Onboarding</span>
                  <span className="text-rk-secondary capitalize">{user.onboarding_state.replace("_", " ")}</span>
                </div>
                <div className="flex flex-col gap-0.5">
                  <span className="text-rk-tertiary uppercase tracking-wide text-[10px] font-[600]">Archetype</span>
                  <span className="text-rk-secondary capitalize">{user.archetype ?? "—"}</span>
                </div>
              </div>

              {user.bio && (
                <p className="text-[12px] text-rk-muted italic border-t border-rk-stroke pt-3">{user.bio}</p>
              )}

              {user.ban_reason && (
                <div className="rounded-[8px] border border-red-900/40 bg-red-900/10 px-3 py-2.5">
                  <p className="text-[11px] text-rk-tertiary uppercase tracking-wide font-[600] mb-1">Ban reason</p>
            ...
            <div className="rounded-[12px] border border-rk-stroke bg-rk-surface p-5 flex flex-col gap-3">
              <div>
                <p className="text-[14px] font-[600] text-rk-primary">Impersonate (view only)</p>
                <p className="text-[12px] text-rk-muted mt-0.5">
                  Step into this user&apos;s session to see exactly what they see. All writes are blocked. Session is audited.
                </p>
              </div>

              {!isSuperAdmin ? (
                <p className="text-[12px] text-rk-muted">Requires super_admin role.</p>
              ) : user.role === "admin" || user.role === "super_admin" ? (
                <p className="text-[12px] text-rk-muted">Cannot impersonate admin accounts.</p>
              ) : user.impersonation_opt_out ? (
                <p className="text-[12px] text-rk-muted">This user has opted out of impersonation.</p>
              ) : user.id === adminId ? (
                <p className="text-[12px] text-rk-muted">Cannot impersonate yourself.</p>
              ) : (
                <button
              ...
              {canImpersonate && (
                <p className="text-[11px] text-rk-tertiary">
                  You will be redirected to the feed viewing as @{user.username}. A vivid banner will be visible at all times.

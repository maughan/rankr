...
import { ItemDistribution } from "@/app/components/item/ItemDistribution";
import { TierItem, type Tier, type ItemRanking } from "@/app/types";
import {
  computeItemDistribution,
  divisiveness,
  type ItemDistribution as ItemDistributionData,
} from "@/lib/itemDistribution";
import TierComparison from "@/app/components/tierComparison";
...
export default function ListDetail({
  ...
  // Community-view inline distribution expander — id of the item whose
  // distribution row is currently open (null = none).
  const [expandedItemId, setExpandedItemId] = useState<number | null>(null);
  // Community-view sort toggle — when on, items sort by divisiveness (sd) desc.
  const [sortByDivisive, setSortByDivisive] = useState(false);
  ...
  // Per-item distribution map — counts by tier value → ItemDistribution.
  // Shared source of truth for the contention dot, inline expander, and the
  // most-divisive callout's spread (sd).
  const distMap = useMemo<Map<number, ItemDistributionData>>(() => {
    const map = new Map<number, ItemDistributionData>();
    if (!list) return map;
    const sortedTiers = [...(list.tiers as Tier[])]
      .filter((t) => t.value > 0) // exclude the N/A (value 0) tier
      .sort((a, b) => b.value - a.value);
    for (const item of list.items as TierItem[]) {
      const countsByValue = new Map<number, number>();
      for (const r of (item.rankings ?? []) as ItemRanking[]) {
        if (r.value === 0) continue;
        countsByValue.set(r.value, (countsByValue.get(r.value) ?? 0) + 1);
      }
      map.set(item.id, computeItemDistribution(countsByValue, sortedTiers));
    }
    return map;
  }, [list]);
  ...
  const divisiveItem = useMemo(() => {
    ...
    for (const item of list.items) {
      ...
      // Spread is sourced from the shared per-item distribution helper rather
      // than recomputed inline.
      const sd = distMap.get(item.id)?.sd ?? 0;
    ...
    return best;
  }, [list, distinctRankerCount, distMap]);
  ...
  // Per-item agreement data — shown when viewing any specific user's ranking.
  // Uses that user's placement vs the crowd, so the badge meaning is consistent
  // whether you're looking at your own ranking or someone else's.
  ...
  >(() => {
    ...
    >();
    if (!list || !userfilter) return empty;
    ...
    for (const item of list.items) {
      ...
      const subjectRanking = allRankings.find((r) => r.user?.id === userfilter);
      if (!subjectRanking) {
        result.set(item.id, null);
        continue;
      }
    ...
    return result;
  }, [list, userfilter, distinctRankerCount]);
  ...
  // Community (aggregate) view = no user filter. The inline distribution
  // expander, contention dots and divisive sort are scoped to this view only.
  const isCommunityView = !userfilter;

  // The viewer's own ranking value for an item (for the "you" marker in the
  // inline distribution). Read straight off the item's rankings.
  const yourValueForItem = (itemId: number): number | null => {
    ...
    <>
      ...
      <div className=" z-10 bg-rk-page overflow-y-auto sm:pb-24">
        ...
        <div className="px-4 sm:px-8 py-6 flex flex-col gap-6 max-w-3xl mx-auto">
          ...
          {/* ── Tier-row controls — community view only ───────────────────── */}
          {list.items.length > 0 && isCommunityView && (
            <div className="flex items-center justify-end -mb-2">
              <button
                onClick={() => setSortByDivisive((v) => !v)}
                className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-[8px] text-[12px] cursor-pointer font-[500] transition-colors ${
                  sortByDivisive
                    ? "bg-rk-accent text-white"
                    : "text-rk-secondary hover:text-rk-primary"
          ...
          {list.items.length > 0 && (
            <div className="flex flex-col gap-[6px]">
              {filteredListRankings.map((tier) => {
                ...
                // The expanded item, if it belongs to this tier (community only).
                const expandedItem =
                  isCommunityView && expandedItemId !== null
                    ? tierItems.find((i) => i.id === expandedItemId)
                    : undefined;

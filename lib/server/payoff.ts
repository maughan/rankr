...
export async function computePayoff({
  ...
  const [
    ...
    // Viewer's precomputed cross-stack taste twin (read via `as any` — the
    // generated client is stale in-sandbox re: `taste_matches`).
    currentUserId !== null
      ? ((prisma.user as any).findUnique({
          where: { id: currentUserId },
          select: { taste_matches: true },
        }) as Promise<{ taste_matches: { twin?: PayoffData["tasteTwin"] } | null } | null>)
      : Promise.resolve(null),
  ]);

  const tasteTwin: PayoffData["tasteTwin"] = twinRow?.taste_matches?.twin ?? null;

...
function MutualsLine({ mutuals }: { mutuals: MutualsInfo }) {
  ...
  if (names.length === 1 && extra === 0) {
    ...
    text = `Followed by ${names[0]}, ${names[1]}, and ${extra} other${
      extra !== 1 ? "s" : ""
    } you follow`;
  }

  return <p className="text-[12px] text-rk-muted leading-snug">{text}</p>;
}
...
function TasteCard({
  ...
  twin: TasteMatchPreview | null;
  nemesis: TasteMatchPreview | null;
}) {
  return (
    <div className="p-4 rounded-[10px] border border-rk-stroke bg-rk-surface flex flex-col gap-2">
      ...
      {twin && (
        <Link
          ...
          className="text-[13px] text-rk-secondary hover:text-rk-primary transition-colors leading-snug"
        >
          <span className="font-[600] text-rk-primary">Taste twin</span> — @
          {twin.username} · agree {twin.pct}% across {twin.sharedLists} list
          {twin.sharedLists !== 1 ? "s" : ""}
        </Link>
      )}
      {nemesis && (
        <Link
          href={`/u/${nemesis.username}`}
          className="text-[13px] text-rk-secondary hover:text-rk-primary transition-colors leading-snug"
        >
          <span className="font-[600] text-rk-primary">Nemesis</span> — @
          {nemesis.username} · clash {nemesis.pct}% across {nemesis.sharedLists}{" "}
          list{nemesis.sharedLists !== 1 ? "s" : ""}
        </Link>
      )}
    </div>
  );
...
function TierChip({ label, color }: { label: string; color?: string }) {
  ...
  return (
    ...
    >
      <span className="text-[11px] font-[700]" style={{ color: bg }}>
        {label}
      </span>
    </div>
  );
...
function ReceiptRow({
  ...
  receipt: ArchetypeReceipt;
  archetypeColor: string;
}) {
  return (
    <div className="flex items-center gap-3 p-2.5 rounded-[8px] bg-rk-page border border-rk-stroke">
      <div className="flex-1 min-w-0">
        <p className="text-[12px] font-[500] text-rk-primary truncate">
          {receipt.itemName}
        </p>
        <p className="text-[11px] text-rk-muted truncate">
          on {receipt.listName}
        </p>
      </div>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <p
                className="text-[18px] font-[700] leading-tight"
          ...
          <p className="text-[12px] text-rk-tertiary">
            ...
            {" · "}
            {stats.rankedItemCount} items across {stats.rankedListCount} list
            {stats.rankedListCount !== 1 ? "s" : ""}
          </p>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-0.5 flex-1 min-w-0">
        <p className="text-[13px] font-[600] text-rk-primary leading-none">
          {config.name}
        </p>
...
function FormingState() {
  return (
    <div className="p-4 rounded-[10px] border border-dashed border-rk-stroke flex flex-col gap-1.5">
      <p className="text-[13px] font-[500] text-rk-secondary">
        {S.archetypeForming.heading}
      </p>
      <p className="text-[12px] text-rk-muted">{S.archetypeForming.subhead}</p>
      <Link
        href="/browse"
        className="text-[12px] text-rk-accent hover:underline mt-0.5 w-fit"
      >
...
function ProfileHero({
  ...
  const memberSince = formatDistanceStrict(new Date(createdAt), new Date(), {
    addSuffix: false,
  });
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-5 items-start sm:items-center py-8">
      ...
      <div className="flex flex-col gap-1.5 flex-1 min-w-0">
        ...
        {/* Bio */}
        {bio && (
          <p className="text-[14px] text-rk-secondary leading-snug max-w-lg">
            {bio}
          </p>
        )}
        ...
        <div className="flex items-center gap-3 mt-0.5 flex-wrap">
          ...
          >
            <span className="text-rk-secondary font-[500]">
    ...
    <div className="animate-pulse flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div
            key={i}
            className="bg-rk-surface border border-rk-stroke rounded-[10px] overflow-hidden"
          >
...
export default function ProfileClient({ username }: { username: string }) {
  ...
  useEffect(() => {
    trackEvent(E.PROFILE_VIEWED, {
      is_own_profile: viewer.username === username,
    });
  ...
  // ── Private profile (visitors only) ─────────────────────────────────────────
  if (is_private && !isOwner) {
    const name = user.display_name ?? user.username;
    return (
      <div className="min-h-screen bg-rk-page flex flex-col">
        {nav}
        <div className="max-w-5xl mx-auto px-4 pb-16 w-full flex-1 flex flex-col items-center justify-center text-center gap-4 py-24">
          ...
          <div className="flex flex-col gap-1 items-center">
            ...
            {user.display_name && (
              <p className="text-[13px] text-rk-muted">@{user.username}</p>
            )}
          </div>
          <p className="text-[14px] text-rk-muted">This profile is private.</p>
        </div>
        <LandingFooter />
      </div>
    );
      ...
      <div className="max-w-5xl mx-auto px-4 pb-16 w-full flex-1">
        <ProfileHero
          ...
          listCount={
            isOwner
              ? lists.length
              : lists.filter((l) => l.visibility === "public").length
          }
        ...
        {/* You-two taste agreement — visitors only */}
        {!isOwner && you_two && (
          <p className="text-[12px] text-rk-muted -mt-3 mb-5">
            You two agree {you_two.pct}% across {you_two.sharedLists} list
            {you_two.sharedLists !== 1 ? "s" : ""}
          </p>
        )}

        {/* Your taste matches — owner only */}
        {isOwner && taste_matches && (
          <div className="mb-5">
            <TasteCard
              twin={taste_matches.twin}
              nemesis={taste_matches.nemesis}
            />
          </div>
        )}
          ...
          <div className="flex gap-1 mb-5 border-b border-rk-stroke">
            {tabs.map(({ key, label }) => {
              const count =
                key === "all"
                  ? lists.length
                  : lists.filter((l) => l.visibility === key).length;
                ...
                >
                  ...
                  <span
                    className={`text-[11px] tabular-nums ${
                      active ? "text-rk-secondary" : "text-rk-tertiary"
                    }`}
                  >
                    {count}
                  </span>

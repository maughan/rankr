import { parseSlugParam, isValidShortId, slugify } from "../lib/listUrl";
...
const cases: [string, string, string][] = [
  ...
  ["old-slug-Kgsbm47R",        "stale slug (will differ in DB)",   "redirect → canonical"],
...
function classifyParam(param: string): string {
  ...
  if (parsed) return `slug+shortId  shortId=${parsed.shortId} slug="${parsed.slug}"`;
...
const slugCases = [
  ...
  ["word ".repeat(12).trim(),   "word-word-word-word-word-word-word-word-word-word-word-word".slice(0,60)],
];
for (const [input, expected] of slugCases) {
  ...
  const pass = typeof expected === "number"
    ...
    : result === expected;
  console.log(`${pass ? "✓" : "✗"} slugify(${JSON.stringify((input as string).slice(0,20))}) → "${result}"`);
  if (!pass) ok = false;
}

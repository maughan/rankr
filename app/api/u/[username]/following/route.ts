...
// GET /api/u/:username/following?cursor=<ISO>&limit=<n>
...
export async function GET(
  ...
  const users = page.map((row: any) => ({
    ...
    viewerFollowsThem: viewerId ? row.following.followers?.length > 0 : false,
    theyFollowViewer: viewerId ? row.following.following?.length > 0 : false,
  }));

...
export async function POST(req: Request) {
  try {
    ...
    // profile_private: must be a boolean if present; undefined means "leave alone"
    if ("profile_private" in data && typeof data.profile_private !== "boolean") {
      return new Response("profile_private must be a boolean.", { status: 422 });
    }
    const profilePrivateUpdate =
      "profile_private" in data

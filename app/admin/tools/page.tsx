...
import { ShieldAlert, KeyRound, Brain, CheckCircle2, XCircle } from "lucide-react";
...
function ToolCard({
  ...
  return (
    <div className={`rounded-[12px] border bg-rk-surface p-5 flex flex-col gap-4 ${
      ...
      <div className="flex items-start gap-3">
        <div
          ...
          style={{
            backgroundColor: danger ? "rgba(239,68,68,0.1)" : "rgba(74,138,232,0.1)",
        ...
        <div>
          <p className="text-[14px] font-[600] text-rk-primary leading-snug">{title}</p>
          <p className="text-[12px] text-rk-muted mt-0.5 leading-snug">{description}</p>
        </div>
      </div>
      ...
      {status === "confirming" && confirmPrompt && (
        <div className="rounded-[8px] border border-amber-800/40 bg-amber-900/10 px-3 py-3 flex flex-col gap-3">
          <p className="text-[12px] text-amber-300 leading-snug">{confirmPrompt}</p>
      ...
      {status === "done" && result && (
        <div className="flex items-start gap-2 rounded-[8px] border border-green-900/40 bg-green-900/10 px-3 py-2.5">
          ...
          <div>
            <p className="text-[12px] font-[500] text-green-300">{result.label}</p>
            <p className="text-[11px] text-green-400/70 mt-0.5">{result.detail}</p>
          </div>
        </div>
      )}
...
export default function AdminToolsPage() {
  ...
  const handleExpireTokens = async (): Promise<ActionResult> => {
    const res = await fetch("/api/admin/tools/expire-tokens", { method: "POST" });
    ...
    return {
      label: `${data.affected} session${data.affected !== 1 ? "s" : ""} expired.`,
      detail: "Everyone has been signed out, including you. Redirecting to login…",
    };
  };

  const handleRecomputeArchetypes = async (): Promise<ActionResult> => {
    const res = await fetch("/api/admin/tools/recompute-archetypes", { method: "POST" });
    if (!res.ok) throw new Error("Request failed.");
    const data = await res.json();
    const errNote = data.errors.length > 0 ? ` (${data.errors.length} failed)` : "";
    return {
      label: `${data.processed} user${data.processed !== 1 ? "s" : ""} processed.`,
